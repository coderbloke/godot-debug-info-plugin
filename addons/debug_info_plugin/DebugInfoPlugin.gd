...
var editor_log = preload("debug_info_editor_log.tscn").instantiate()

func init_singleton():
	singleton = get_node_or_null("/root/" + AUTOLOAD_NAME)
	if singleton != null:
		singleton.info_panel = info_panel
		singleton.editor_log = editor_log
...
func _enter_tree():
	...
	add_control_to_bottom_panel(editor_log, "DebugInfoLog")
...
func _exit_tree():
	if singleton != null:
		...
		singleton.editor_log = null
	remove_control_from_docks(info_panel)
	remove_debugger_plugin(debugger_plugin)
	remove_control_from_bottom_panel(editor_log)
